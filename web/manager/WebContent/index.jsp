...
<head>
    ...
    <c:choose>
        ...
        and (sessionSysUser.subsysCode eq 'merchant' or sessionSysUser.subsysCode eq 'distributor')
    ...
    <script type="text/javascript">
        curl(['zeroClipboard', 'UE', 'clipboard'], function (zeroClipboard, UE, Clipboard) {
            ...
            //用于区分不同后台的访问路径
            window.managerUrl = root + "/" + $('input[name="currentSubsysCode"]').val();
        });
...
//            cometNew = new CometNew();
    ...
    <script type="text/html" id="leftMenuArtTmpl">
        ...
        <li id="menuItem{{obj.object.id}}" >
            <a  href="#">
                <em class="">
                    <img src="${resRoot}/images/{{obj.object.resourceIcon}}" style="font-size:13px;opacity: .7; margin-top: -5px;">
                </em>
            ...
            <ul class="nav nav-second-level collapse" aria-expanded="false" >
                ...
                <li><a {{if child.object.resourceUrl == null}} href='/index/content.html?parentId={{child.object.id}}' {{else}} href='/{{child.object.resourceUrl}}' {{/if}}
...
<div id="wrapper">
    <div class="top">
        ...
        <div class="pull-right top-account-menu">
            ...
            <ul class="tips">
                <c:if test="${(sessionSysUser.subsysCode) == 'merchant' }">
                    <li class="tasks">
                        <input id="isReminderTask" type="hidden" value=""/>
                        <a href="javascript:void(0)" class="locate" data-toggle="dropdown"  data-href="${root}/vTaskReminder/index/task.html">
                            　<i class="icon iconfont"></i><span class="label label-orange" id="unReadTaskCount">0</span></a>
                    ...
                    <li class="informations">
                        ...
                        <dl class="infos_list nav-shadow">
                            <dd class="infos-none t-left">
                                <div>当前时区：<%= SessionManager.getTimeZone().getID() %></div>
                                <div>当前时间：<div id="index-clock" class="clock-show"></div></div>
                ...
                <li class="infos show-info">
                    ...
                    <ul class="information nav-shadow">
                        <shiro:hasPermission name="${subsysCode}:my_account">
                            <li>
                                <a href="/${subsysCode}/account/mine/index.html" nav-target="mainFrame" add-table="addTable">
                                    <i class="fa fa-user"></i>我的账户
                                </a>
                            </li>
                        </shiro:hasPermission>
                        <shiro:hasPermission name="${subsysCode}:my_role">
                            <li>
                                <a href="/${subsysCode}/account/role/list.html" nav-target="mainFrame" add-table="addTable">
                        ...
                        <shiro:hasPermission name="system:log">
                            <li>
                                <a href="/${subsysCode}/system/log/logList.html?search.roleType=boss&search.operator=${username}" nav-target="mainFrame"
                                   add-table="addTable"><i class="fa fa-file-excel-o"></i>系统日志</a>
                            </li>
                        </shiro:hasPermission>
                        <shiro:hasPermission name="${subsysCode}:update_my_login_pwd">
                            <li>
                ...
                <div class="navbar-header">
                    <div href="javascript:void(0)" class="navbar-brand">
                        ...
                        <c:choose>
                            ...
                            and (sessionSysUser.subsysCode eq 'merchant' or sessionSysUser.subsysCode eq 'distributor')
                            and logo ne 'Logo/1/1459172297237.png'}">
                                <img style="margin-left: 40px" data-url="${soulFn:getImagePath(domain, logo)}" src="${soulFn:getThumbPath(domain, logo, 100, 50)}">
                            </c:when>
                            <c:when test="${not empty logo and (sessionSysUser.subsysCode eq 'boss' or sessionSysUser.subsysCode eq 'shareholder')}">
                                <img src="${resRoot}/images/logo.png"/>
                            </c:when>
                            <c:when test="${empty logo or logo eq 'Logo/1/1459172297237.png'}"><span style="margin-left: 40px">${siteTilte}</span></c:when>
                        </c:choose>
                    </div>
                    <%--时间显示暂时取消--%>
