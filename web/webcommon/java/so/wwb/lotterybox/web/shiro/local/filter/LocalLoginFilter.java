...
import so.wwb.lotterybox.model.enums.sys.UserTypeEnum;
...
//            }
            ...
            //TODO dick添加   || DomainTypeEnum.MERCHANT == contextParam().getDomainTypeEnum()
            if (DomainTypeEnum.MANAGER == contextParam().getDomainTypeEnum() || DomainTypeEnum.MERCHANT == contextParam().getDomainTypeEnum()) {
...
//        if (SubSysCodeEnum.DISTRIBUTOR.getCode().equalsIgnoreCase(contextParam().getDomainSubsysCode())) {
...
//        boolean isOpenCaptcha = isOpenCaptcha(request);
    ...
    protected PassportResult isValidToken(HttpServletRequest request, LocalToken token) {
        ...
        //游戏中心暂时不开启验证码. // TODO:alvin 2017/7/29
        if (DomainTypeEnum.HALL == contextParam().getDomainTypeEnum()) {
            passportResult.setIsOpenCaptcha(Boolean.FALSE);
        } else {
//            passportResult.setIsOpenCaptcha((Boolean) SessionManagerCommon.getAttribute(SessionKey.S_IS_CAPTCHA_CODE));
            passportResult.setIsOpenCaptcha(false); // TODO: add by Fei 取消验证码校验
        }
