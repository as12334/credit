...
public class SysDomainService extends BaseService<SysDomainMapper, SysDomainListVo, SysDomainVo, SysDomain, Integer> implements ISysDomainService {
    ...
    public SysDomainListVo setting(SysDomainListVo sysDomainListVo) {
        Criteria.add(SysDomain.PROP_IS_FOR_ALL_RANK, Operator.EQ, true);
        Criteria.add(SysDomain.PROP_SUBSYS_CODE, Operator.EQ, true);
        mapper.search(Criteria.add(SysDomain.PROP_IS_FOR_ALL_RANK, Operator.EQ, true), Order.asc(SysDomain.PROP_SORT));
        return null;
    }
    ...
    public int batchUpdateStatusToFalse(SysDomainVo sysDomainVo) {
        ...
        return mapper.batchUpdateProperties(Criteria.add(SysDomain.PROP_IS_DEFAULT, Operator.EQ, true)
                ...
                .addAnd(SysDomain.PROP_TYPE, Operator.EQ, sysDomainVo.getResult().getType())
                , map);
    }
    ...
    public SysDomainVo batchSaveDomain(SysDomainVo sysDomainVo) {
        ...
        if(sysDomainVo.getResult().getIsDefault() !=null && sysDomainVo.getResult().getIsDefault()){
            ...
            if (sysDomainVo.getResult().getTemplateCode() == null || StringTool.isBlank(sysDomainVo.getResult().getTemplateCode())){
                sysDomainVo.getResult().setTemplateCode(sysSiteVo.getResult().getTemplateCode());
            }
            if (sysDomainVo.getResult().getTheme() == null || StringTool.isBlank(sysDomainVo.getResult().getTheme())){
                sysDomainVo.getResult().setTheme(sysSiteVo.getResult().getTheme());
            }
    ...
    public SysDomainVo saveAgentDomain(VSysDomainVo objectVo) {
        ...
        domain.setType(vSysDomain.getType());
